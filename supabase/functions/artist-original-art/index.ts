...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      // Insert record with owner_user_id set to artist's ID
      const { data: artItem, error: dbError } = await supabase
        ...
        .insert({
          ...
          provenance,
          visibility: 'private',
          owner_user_id: user.id, // Set to artist's ID
        })
        .select()
        .single();
      ...
      console.log('Original art created by artist:', artItem.id);
    ...
    // PATCH - Update own art (cannot change visibility)
    if (req.method === 'PATCH' && artId) {
      ...
      // Remove visibility from update if present
      ...
      console.log('Original art updated by artist:', artId);
    ...
    console.error('Error in artist-original-art:', error);
